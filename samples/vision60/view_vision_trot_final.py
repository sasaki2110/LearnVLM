...
# 1. スポーン位置を低くし、安定しやすい高さにする（学習時と同じ）
spawn_height = 0.3
...
for i, j_idx in enumerate(joint_indices):
    ...
    # 初期状態で関節がヘニャっとならないよう、モーター制御をかけておく（学習時と同じ）
    p.setJointMotorControl2(robot_id, j_idx, p.POSITION_CONTROL, 
    ...
    if i in [2, 5, 8, 11]:
        print(f"  初期姿勢: ジョイント {j_idx} (Knee) を {knee_angle:.2f} rad に設定")

# 3. ★ここがポイント：安定を待つ（ウォームアップ、学習時と同じ）
...
# --- 2. モデルの読み込み ---
model_path = "ppo_vision60_trot_final"
try:
    # .zip拡張子を試す
    try:
    ...
    print(f"   先に train_vision_trot_final.py を実行してモデルを学習してください")
    exit(1)
...
print("   Vision60ロボットのトロット（足踏み）動作を表示します（final版）")
print("   📹 カメラがロボットを自動追跡します")
print("   📝 状態を vision60_flight_log_final.csv にログ記録します\n")
input("⏸️  Enterキーを押すと再生を開始します...")

# --- 4. ログファイルの準備 ---
log_filename = 'vision60_flight_log_final.csv'
...
with open(log_filename, mode='a', newline='') as log_file:
    ...
    while True:
        ...
        for i, j_idx in enumerate(joint_indices):
            # 基準となるトロットの動きを計算（学習時と同じロジック）
            target_pos = 1.2  # 学習時と同じ基準位置
            if i in [2, 8]:  # FR, RL の Knee
                target_pos += phase_a * 0.4  # 学習時と同じ振幅
            elif i in [5, 11]:  # FL, RR の Knee
                target_pos += phase_b * 0.4  # 学習時と同じ振幅
            
            # AIのアクションを「補正値」として加える
            target_pos += action[i] * 0.2
            
            # 可動範囲制限（学習時と同じ：0.1~3.0）
            target_pos = np.clip(target_pos, 0.1, 3.0)
            
            p.setJointMotorControl2(
                robot_id, j_idx, p.POSITION_CONTROL,
                targetPosition=target_pos, force=100.0  # 学習時と同じ力
                ,
                positionGain=0.05,   # 標準の半分にして「柔らかく」
                velocityGain=1.5     # 少し増やして「跳ね」を抑える
            )
    ...
    """
    if height < 0.35 or abs(roll) > 0.4 or abs(pitch) > 0.5:
        print(f"⚠️  転倒を検出。リセットします... (高さ: {height:.3f}, roll: {roll:.3f}, pitch: {pitch:.3f})")
        p.resetBasePositionAndOrientation(robot_id, [0, 0, 0.55], [0, 0, 0, 1])
        # ジョイントもリセット（初期姿勢に戻す）
        for i, j_idx in enumerate(joint_indices):
            if i in [2, 5, 8, 11]:  # 膝を少し曲げる
                p.resetJointState(robot_id, j_idx, 1.2)
            else:
